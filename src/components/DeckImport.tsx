...
import { Card, Cards, Paragons } from "~/types/sharedTypes";
...
export default function DeckImport(props: DeckImportProps) {
  ...
  const [importError, setImportError] = useAtom(importErrorAtom);
  ...
  const handleDeckDode = () => {
    try {
      ...
      let trimmedDeckCode = deckCode?.trim();
      ...
      if (paragonIds.includes(deckCodeArr![0]!)) {
        const importedParagon = paragons.filter(
          (paragon) => paragon.id === deckCodeArr![0]
        );
        setParallelChoice(importedParagon![0]!.gameData.parallel);
        setActiveParagon(importedParagon![0]!);
      }
      // create arr without paragon included
      let cardEntries = [...deckCodeArr];
      cardEntries.shift();
      ...
      const newArray = cardEntries.flatMap((item) => {
        ...
        const matchingCards = cards.reduce(
          (uniqueCards: Cards, currentCard) => {
            if (
              currentCard.id === id &&
              !uniqueCards.some((card: Card) => card.id === id)
            ) {
        ...
        return matchingCards.flatMap((obj) =>
          Array.from({ length: quantity }, () => ({ ...obj }))
        );
      });
    ...
    <Transition.Root show={openImport} as={Fragment}>
      ...
      >
        <div className="flex min-h-screen items-end justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
          ...
          >
            <div className="bg-nuetral-100 inline-block transform overflow-hidden rounded-lg bg-neutral-200 px-4 pt-5 pb-4 text-left align-bottom shadow-xl transition-all dark:bg-neutral-900 sm:my-8 sm:w-full sm:max-w-sm sm:p-6 sm:align-middle">
              <div>
                ...
                <div className="mt-3 text-center sm:mt-5">
                  ...
                  <div className="m-2 mb-8 mt-8">
                    <p className="text-sm text-gray-500">
                      *Code must be in the offical Parallel Deck Code format,
                      i.e. CB-9,CB-408,2XCB-103,3XCB-28
                    </p>
                  </div>
                </div>
              </div>
